...
export async function scrapeIncredible(query: string): Promise<ScraperResult[]> {
  ...
  try {
    ...
    // Magento-style selectors (same platform as Wootware)
    const selectors = [
      ...
      { container: ".product-item-info", name: ".product-item-name a", price: ".price, .special-price .price", link: ".product-item-name a" },
      { container: "[class*='product']", name: "a[class*='product']", price: "span[class*='price']", link: "a[href*='/']" },
    ];
    ...
    for (const sel of selectors) {
      $(sel.container).each((_, el) => {
        ...
        if (name && price && price > 100) {
          ...
          results.push({
            retailerId: "incredible",
            retailerName: "Incredible Connection",
            productName: name,
            priceZAR: price,
            productUrl: href || url,
            inStock: true,
